...
class PromptLearner(nn.Module):
    def __init__(self, args, classnames, clip_model):
        ...
        if ctx_init:
            ...
            # initialize context vectors [n_ctx, ctx_dim]
    ...
    def forward(self):
        ...
        if ctx.dim() == 2:
            ...
            assert ctx.shape[0] == self.n_cls, f"ctx.shape[0] ({ctx.shape[0]}) must equal to self.n_cls ({self.n_cls})"
        else:
            raise ValueError(f"ctx.dim() ({ctx.dim()}) must be 2 or 3")

        prefix = self.token_prefix
        suffix = self.token_suffix

        if self.class_token_position == "end":
            # [n_cls, n_ctx + 1, ctx_dim]
            prompts = torch.cat(
                [
                    prefix,  # (n_cls, 1, dim)
                    ctx,  # (n_cls, n_ctx, dim)
