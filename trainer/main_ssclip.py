...
def main():
    ...
    criterion = {}
    criterion['bce'] = nn.BCEWithLogitsLoss(reduce=True, size_average=True).cuda()
    criterion['bce'] = nn.CrossEntropyLoss(reduce=True, size_average=True).cuda()
...
def train(train_loader, model, criterion, optimizer, epoch, args, writer):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        for i, sample in enumerate(output):
            contrastive_loss += loss(sample[label[i].argwhere()].squeeze(), label[i].argwhere().reshape(-1))
